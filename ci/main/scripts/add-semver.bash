...
# ggupgrade-intermediate.el7.x86_64.rpm -> ggupgrade-0.4.0+dev.32.g763a08e5-1.el7.x86_64.rpm

function rename_rpm() {
    local release=$1

    rpm=$(basename "${release}_rpm"/ggupgrade-*.rpm)
    # shellcheck disable=2001
    # NOTE: The "-1" is the packing build number which enables us to release
    # a new package when there are no code changes.
    sed s/ggupgrade-intermediate/ggupgrade-"${SEMVER}-1"/ <<< "$rpm"
}
...
if (( ${#parts[@]} == 1 )); then
    SEMVER="${parts[0]}"
    cp oss_rpm/ggupgrade-*.rpm renamed_oss_rpm/"$(rename_rpm oss)"
    cp enterprise_rpm/ggupgrade-*.rpm renamed_enterprise_rpm/"$(rename_rpm enterprise)"
elif (( ${#parts[@]} == 3 )); then
