...
- [Acceptance Tests](#acceptance-tests)
    - [Testing GPDB changes against ggupgrade](#testing-gpdb-changes-against-ggupgrade) 
    - [1) ggupgrade Acceptance Tests](#1-ggupgrade-acceptance-tests)
...
Acceptance tests are "end-to-end" tests that exercise the binary of ggupgrade and
the overall Greengage upgrade process. There are two types of acceptance tests:
1) ggupgrade, and 2) pg_upgrade. And within the pg_upgrade acceptance tests
there are two sub-types: a) non-upgradeable, and b) upgradeable.

---

## Testing GPDB changes against ggupgrade
...
3. Create a ggupgrade test branch and push it. Next, generate a ggupgrade test pipeline that uses the GPDB RC RPMs using 
...
## 1) ggupgrade Acceptance Tests

These tests exercise the end-to-end components and features of ggupgrade from the perspective of the ggupgrade binary.
...
- Run ggupgrade initialize which calls `pg_upgrade --check`.
...
described in the [documentation](https://gpdb.docs.pivotal.io/upgrade/1-0/ggupgrade_initialize_checks.html).
...
they will skip many of the `ggupgrade initialize` substeps which were already run.
...
See [PR #555](https://github.com/GreengageDB/ggupgrade/pull/555) for details.

### pg_upgrade: upgradeable tests (positive tests)

The pg_upgrade upgradeable acceptance tests are positive tests. They assert
that interesting objects can be successfully upgraded by ggupgrade.
...
- Upgrades the cluster with `ggupgrade initialize` and `ggupgrade execute`.
...
- Upgrades the cluster with `ggupgrade initialize`, `ggupgrade execute`, `ggupgrade finalize`.
