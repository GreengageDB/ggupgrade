...
check_synchronized_cluster() {
    ...
    for i in {1..10}; do
        ...
        synced=$(ssh -n "$coordinator_host" "
            source ${GPHOME_NEW}/greengage_path.sh
...
validate_mirrors_and_standby() {
    ...
    if [[ $coordinator_data_dir != *datadirs/qddir/demoDataDir* && $coordinator_data_dir != */data/coordinator/gpseg-1* ]]; then
    ...
    # sanity check both the demo cluster and CI cluster cases
    if [[ $standby_data_dir != *datadirs/standby* && $standby_data_dir != */data/coordinator/gpseg-1* ]]; then
