...
func TestCreateBackupDirectories(t *testing.T) {
	source := hub.MustCreateCluster(t, greengage.SegConfigs{
		...
		{DbID: 7, ContentID: 2, Hostname: "sdw3", DataDir: "/data3/mirrors/seg3", Port: 25437, Role: greengage.MirrorRole},
		{DbID: 8, ContentID: 2, Hostname: "sdw3", DataDir: "/data3/primaries/seg3", Port: 25438, Role: greengage.PrimaryRole},
	})
...
func TestCreateBackupDirectory(t *testing.T) {
	...
	t.Run("creates backup directory if it does not exist", func(t *testing.T) {
		...
		backupDir := filepath.Join(parentBackupDir, ".ggupgrade")
